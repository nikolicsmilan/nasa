// src/components/Charts/area/areachartlineoptions.js (vagy chartOptions.js)
...
export const areachartlineoptions = (colors, config, displayedData) => {
  ...
  return {
    ...
    plugins: { // Beépülő modulok konfigurációja
      tooltip: { // Tooltip (buboréksúgó) beállításai
        enabled: true, // Tooltip engedélyezve
        callbacks: { // Egyedi függvények a tooltip tartalmának generálásához
          label: function (context) { // A fő tooltip címke (általában az érték) formázása
            let datasetLabel = context.dataset.label || ""; // Adatsor neve (pl. "Impacts")
            const yValue = context.parsed?.y; // A pont parse-olt Y értéke (lehet szám vagy Date.getTime())
            ...
            if (yValue !== null && yValue !== undefined) { // Csak akkor formatálunk, ha van érvényes Y érték
              if (isDateAxis && originalItem?._parsedValue instanceof Date) { // Ha dátum tengely és az érték Date objektum...
                try {
      ...
      legend: { // Jelmagyarázat beállításai
        display: false, // Legenda kikapcsolva (mivel csak egy adatsor van)
        labels: { // Címkék stílusa (ha látható lenne)
          color: legendColor,
          boxWidth: 12,
          padding: 15,
          font: { size: 12 },
      ...
      title: { // A diagram fő címének beállításai
    ...
    interaction: { // Interakciók (pl. hover) beállításai
    ...
    animation: { // Animációk beállításai
      duration: 400, // Animáció időtartama ms-ban
